...
function draw() {
	...
	for(var i=0;i<grid.length;++i)
			grid[i].show();
				...
				current.highlight();
				//STEP1 check for unvisited neighbors
...
function index(a,b)
{
	if(a<0||b<0||a>(rows-1)||b>(cols-1))//check for invalid index values for the edge cases
			return -1;
				...
				this.show=function()
					...
					this.checkNeighbors=function()
						{			var neighbors=[];
									...
									if(top&&!top.visited)//including the mid and the edge cases for the neighbors
													neighbors.push(top);
